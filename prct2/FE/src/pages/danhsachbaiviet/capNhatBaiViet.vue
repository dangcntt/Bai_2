...
export default {
  ...
  methods: {
    ...
    RemoveTree(node, instanceId) {
      ...
      if (value != null) {
        this.modelContent.menu = this.modelContent.menu.filter(
          (x) => x.id != value.id
        );
    ...
    async GetTreeList() {
      await this.$store
        .dispatch("menuCongDanStore/getTreeListTBV")
        .then((res) => {
    ...
    async handleUpdate() {
      await this.$store
        .dispatch("contentStore/getById", { _id: this.$route.params.id })
        .then((res) => {
          if (res.code === 0) {
            console.log(res);
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        await this.$store
          .dispatch(
            "contentStore/update",
            contentModel.toJson(this.modelContent)
          )
          .then((res) => {
            if (res.code === 0) {
              this.GetTreeList();
    ...
    async upload() {
      if (this.modelContent != null && this.modelContent.fileImage != null) {
        ...
        axios
          .post(
            `${process.env.VUE_APP_API_URL}filesminio/delete/${this.modelContent.fileImage.fileId}`
          )
          .then((response) => {
      ...
      if (event.target && event.target.files.length > 0) {
        ...
        axios
          .post(
            `${process.env.VUE_APP_API_URL}filesminio/UploadFileImage`,
            formData
          )
          .then((response) => {
    ...
    getColorWithExtFile(ext) {
      ...
      else if (ext == ".xlsx" || ext == ".xls") return "icon-excel";
      else if (ext == ".pdf") return "icon-pdf";
    ...
    deleteFile(id) {
      ...
      console.log("log model file remove", this.modelContent.files);
      axios
        .post(`${process.env.VUE_APP_API_URL}filesminio/delete/` + id)
        .then((response) => {
          console.log("log model file remove", this.modelContent.files);
        })
        .catch((error) => {
      ...
      <div class="card-body">
        <form @submit.prevent="handleSubmit" ref="formContainer">
          <div class="row" ref="formContainer">
            ...
            <div class="col-12">
              <div class="mb-2">
                ...
                <treeselect
                  ...
                  :class="{
                    'is-invalid': submitted && $v.modelContent.menu.$error,
                  }"
                >
                  <label
                    slot="option-label"
            ...
            <div class="col-3">
              <div class="mb-2">
                <label for="formFileSm" class="text-left mb-0"
                  >Ảnh đại diện</label
                >
                <input
                  id="formFileSm"
                ...
                <a
                  ...
                  ><i
                    :class="`${getColorWithExtFile(
                      modelContent.fileImage.ext
                    )} me-2 ${getIconWithExtFile(modelContent.fileImage.ext)}`"
                  ></i
                  >{{ modelContent.fileImage.fileName }}</a
                >
                <b-button
                  variant="flat-danger"
                  class="btn-icon"
            ...
            <template v-if="modelContent.files.length > 0">
              <div v-for="(file, index) in modelContent.files" :key="index">
                <a class="ml-25" :href="`${urlFile}/${file.fileId}`"
                  ><i
                    :class="`${getColorWithExtFile(
                      file.ext
                    )} me-2 ${getIconWithExtFile(file.ext)}`"
                  ></i
                  >{{ file.fileName }}</a
                >
                <b-button
