...
export default {
  ...
  methods: {
    ...
    async getTreeFlatten() {
      await this.$store
        .dispatch("menuCongDanStore/getTreeFlatten")
        .then((res) => {
          localStorage.setItem("flatten-menu", JSON.stringify(res.data));
        });
    },
    async getNoiBo() {
      await this.$store.dispatch("menuStore/getTreeFlatten").then((res) => {
        localStorage.setItem("menu-noibo", JSON.stringify(res.data));
      });
    },
    ...
    async Login(e) {
      ...
      if (this.$v.$invalid) {
        ...
        await this.$store
          ...
          .then(async (res) => {
            if (res.code === 0) {
              console.log("LOG NGUYEN QUOC VU ", res);
  ...
  <div class="position-relative">
    <div class="navbar-expand-lg fixed-top">
      <nav class="navbar pd-0 bg-menu" id="navbar" style="padding: 0px">
        <div class="container cs-navbar-header menu">
          <a
            href="/"
            class="navbar-logo d-flex justify-content-center align-items-center"
          >
            <!-- <img src="@/assets/images/half.png" alt height="40" class="logo logo-dark"/>
            <img src="@/assets/images/half.png" alt height="40" class="logo logo-light"/> -->
            <div class="cs-title-header" style="margin-left: 10px">
              <p class="text-white text-uppercase mb-0">Bài Thi</p>
            </div>
          </a>
          <button
            type="button"
          ...
          >
            <i
              class="mdi mdi-format-align-justify pd-5"
              style="color: #fff"
            ></i>
          </button>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <b-dropdown-item
                    v-if="
                      currentUserAuth != null && currentUserAuth.menu != null
                    "
                    @click="handlePush('/danh-sach-bai-viet')"
                    style="width: 100%"
                  >
                  ...
                  <b-dropdown-item
                    @click="handlePush('/thong-tin-ca-nhan')"
                    style="width: 100%"
                  >
                    <i class="bx bx-user font-size-16 align-middle me-1"></i>
                    Thông tin cá nhân
      ...
      <footer class="landing-footer bg-footer bg-menu">
        ...
        >
          <Transition name="fade" mode="out-in">
            <div
              v-if="!showRegisterModal && !showForgotModal"
              class="row justify-content-center"
            >
              <div class="col-md-12">
                <div
                  class="card overflow-hidden"
                  style="padding: 0px; margin-bottom: 0px; box-shadow: none"
                >
                  <div class="bg-soft bg-primary bg-login"></div>
...
.bg-navbar-dark {
  background: rgba(0, 149, 144, 0.56);
}
