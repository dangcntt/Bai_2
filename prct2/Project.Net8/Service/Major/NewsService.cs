    ...
    public class NewsService : BaseService, INewsService
    {
        ...
        public async Task<dynamic> GetByMenuId(PagingParamDefault pagingParam)
        {
            try
            {
                ...
                MenuCitizenModel menu = _context.MENUCITIZEN.Find(x => !x.IsDeleted && x.Id == pagingParam.MenuId)
                    .FirstOrDefault();
                if (menu != null && menu.SoLuongTin == 1)
                {
                    return _context.NEWS.Find(x => !x.IsDeleted && x.IsPublish && x.Menu.Id == pagingParam.MenuId)
